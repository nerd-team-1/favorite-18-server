...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        log.info("Authentication Interceptor url : {}", request.getRequestURI());
        ...
        UserDto userDto = jwtBusiness.validationAccessToken(accessToken);

        if(!ObjectUtils.isEmpty(userDto)) {
            final RequestAttributes requestContext = Objects.requireNonNull(RequestContextHolder.getRequestAttributes());
            requestContext.setAttribute("userId", userDto.getId(), RequestAttributes.SCOPE_REQUEST);
            requestContext.setAttribute("userRole", userDto.getRole(), RequestAttributes.SCOPE_REQUEST);
